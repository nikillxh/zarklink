...
│
├── scripts/
│   ├── start-devnet.sh              # Infrastructure orchestrator (~1400 lines)
│   ├── deploy.sh                    # Contract build + deploy wrapper
│   ├── deploy.ts                    # TypeScript deployment script (devnet)
│   ├── deploy-sepolia.ts            # Sepolia testnet deployment
│   ├── create-sepolia-account.ts    # Sepolia account generator
│   ├── devscript0-setup.ts          # Vault registration + collateral setup (8 vaults, devnet)
│   ├── devscript1-simulate.ts       # Multi-issue + redeem simulation (devnet)
│   ├── tntscript0-setup.ts          # Vault registration + collateral setup (testnet)
│   ├── tntscript1-simulate.ts       # Issue + redeem walkthrough (testnet)
│   ├── test-functional.ts           # End-to-end functional test (18 assertions)
│   ├── push-to-prod.sh              # Push to production helper
...
git clone <repo-url> zarklink && cd zarklink
...
|------|---------|-------------|
| `.env.devnet` | Devnet RPC URLs, account keys, contract addresses | `start-devnet.sh` + `deploy.ts` |
| `frontend/.env.local` | Active frontend config (copied from devnet/testnet) | `switch-env.sh` or manual |
| `frontend/.env.devnet` | Devnet frontend config backup | `deploy.ts` (auto-synced) |
| `frontend/.env.testnet` | Sepolia testnet frontend config | `deploy-sepolia.ts` |
| `.devnet/deployments.json` | Full devnet deployment record | `deploy.ts` |
| `.sepolia/deployments.json` | Full Sepolia deployment record | `deploy-sepolia.ts` |
| `.sepolia/account.json` | Sepolia deployer account (address, keys) | `create-sepolia-account.ts` |

### Network Switching

Switch between devnet (localhost) and Sepolia testnet with one command:

```bash
# Switch to devnet (localhost)
./switch-env.sh devnet
...
This copies the appropriate env file to `frontend/.env.local` and the frontend picks it up on restart. The Navbar shows a green "Devnet" or yellow "Sepolia" badge.

> **Note:** Dev tools page (`/dev`) and dev API (`/api/dev`) are only available in devnet mode.
...
Main infrastructure orchestrator. Uses two Zcash RPC helpers internally:
`zcash_rpc()` (stderr visible for debugging) and `zcash_rpc_quiet()` (stderr
suppressed for polling/status checks).
...
### `scripts/devscript0-setup.ts`

Registers 8 vault operators with graduated collateral tiers and deposits collateral
to both VaultRegistry and VaultPool. Displays pool share percentages using
largest-remainder rounding (guaranteeing they sum to exactly 100.00%).

```bash
node_modules/.bin/tsx scripts/devscript0-setup.ts
```

### `scripts/devscript1-simulate.ts`

Runs a multi-step bridge simulation: multiple Issue flows followed by a Redeem,
with relay header advancement between each step. Useful for testing the full
protocol lifecycle.

```bash
node_modules/.bin/tsx scripts/devscript1-simulate.ts
```

### `scripts/tntscript0-setup.ts`

Testnet vault setup. Registers a single vault operator (the deployer) with
collateral on Starknet Sepolia. Unlike devnet, the user can only access their
own account. Requires funded deployer on Sepolia and deployed contracts
(`.env.sepolia` / `.sepolia/deployments.json`).

```bash
npx tsx scripts/tntscript0-setup.ts
```

### `scripts/tntscript1-simulate.ts`

Testnet simulation: walks through a single Issue cycle (request_lock +
submit_mint + confirm_issue) using the deployer's vault. Advances the relay
with headers from the Zcash testnet (if RPC is available). Useful for verifying
the testnet deployment end-to-end.

```bash
npx tsx scripts/tntscript1-simulate.ts
```
...
| Route | Description |
|-------|-------------|
| `/` | Dashboard — bridge stats, relay status, pool info, dual balances (ZEC/TAZ + wZEC/wTAZ) |
| `/bridge` | Issue (ZEC/TAZ → wZEC/wTAZ) and Redeem — devnet auto-transfer, testnet manual send |
| `/vaults` | Vault registry browser with pool share percentages (auto-refreshes every 15s) |
| `/account` | Zcash account association — link Zcash addresses to your Starknet account |
...
| `/api/zcash-balance` | Server-side API — proxies zcashd RPC for balance queries (devnet) or blockchain queries (testnet) |
| `/api/dev` | Server-side API — dev tools (mine blocks, generate/fund addresses, relay seeding, send ZEC) |

### Bridge Page Features

- **Balance Display**: Shows both wZEC/wTAZ (Starknet) and ZEC/TAZ (Zcash) balances side-by-side
- **Real ZEC Transfers (Devnet)**: Actually sends ZEC from issuer to vault via `z_sendmany` during Issue
- **Manual TAZ Send (Testnet)**: Shows vault address and exact amount; user sends TAZ from their own wallet, then clicks "I've Sent the TAZ" to complete
- **Devnet Auto-Completion**: Executes full multi-step protocol (Issue: 3 steps + ZEC transfer, Redeem: 2 steps)
- **Pre-Submit Validation**: Checks both ZEC/TAZ balance (issue) and wZEC/wTAZ balance (redeem) before submission, with inline warnings for insufficient funds
- **Max Button**: Pre-fills amount inputs with your full balance (wZEC for redeem, ZEC for issue)
...
Zcash account association page for linking Zcash addresses to your Starknet account:

- **Associate Addresses**: Link a Zcash shielded or transparent address to your connected Starknet wallet
- **Multiple Associations**: Manage multiple Zcash addresses per Starknet account
- **Active Account Banner**: Shows currently connected Starknet + associated Zcash address
- **TAZ Faucet Links** (testnet): Direct links to Zcash testnet faucets
- **Validation**: Supports `ztestsapling1`, `tm`, `zs1`, `t1`, `t3` address prefixes
- **localStorage Storage**: Associations stored client-side per network (devnet/testnet)
...
2. **Browser Wallet (ArgentX / Braavos)** — Native Starknet browser wallet extensions
3. **MetaMask (via Starknet Snap)** — MetaMask with the [Starknet Snap](https://snaps.consensys.io/starknet) for users who prefer MetaMask

Wallet selection uses [starknetkit](https://www.starknetkit.com/) with a modal that auto-detects available wallets. Switch between devnet and browser modes using the wallet connector in the navbar.

### Network-Aware Naming

The frontend automatically adapts terminology based on the active network:

| Network | Zcash Coin | Wrapped Coin |
|---------|-----------|-------------|
| Devnet (localhost) | ZEC | wZEC |
| Testnet (Sepolia) | TAZ | wTAZ |
| Mainnet | ZEC | wZEC |
...
Full end-to-end functional test on devnet covering vault registration, relay seeding, Issue flow, Redeem flow, and vault tracking verification:

```bash
# Ensure devnet is running with deployed contracts
node_modules/.bin/tsx scripts/test-functional.ts
```

This tests 8 sections (18 assertions):
1. Contract read functions (fee_rate, warranty, vault_count)
2. Vault registration + collateral deposit
3. Relay header seeding (10 headers with Poseidon hash chaining)
4. Issue flow (request_lock → submit_mint → confirm_issue)
5. Vault tracking after issue (`total_issued` updated)
6. Redeem flow (submit_burn → confirm_redeem)
7. Vault tracking after redeem (`total_redeemed` updated)
8. Pool & stats reads (active vaults, issue/redeem counts, wZEC supply)
...
**z_sendmany fails with "Insufficient funds: have 0.00"**
```
This is a Zcash v6+ privacy policy issue. When sending from a transparent address
(t-addr) to a shielded address (z-addr), the transparent change output reveals
both sender and recipients. You must use the AllowFullyTransparent privacy policy:

  zcash-cli z_sendmany "t1..." '[{"address":"z...","amount":5}]' 1 null AllowFullyTransparent

The positional args are: <from> <amounts> <minconf> <fee> <privacyPolicy>.
You must provide fee (use null for default ZIP 317) to reach the privacyPolicy argument.

Privacy policy hierarchy (most → least private):
  FullPrivacy > AllowRevealedAmounts > AllowRevealedRecipients >
  AllowRevealedSenders > AllowFullyTransparent > NoPrivacy
```

**zcashd wallet takes 5+ minutes to load on first start**
```
CWallet::NewKeyPool generates an initial keypool of keys, which takes ~300s
on some systems. The start-devnet.sh script has a 420s (7 minute) timeout
for wallet readiness. This only happens on the first start; subsequent starts
with an existing wallet are much faster.
```

**bc: command not found (shell arithmetic)**
```
The bc calculator is not installed on all systems. start-devnet.sh uses native
bash string comparison instead of bc for balance checking. If you see bc-related
errors in other scripts, replace:
  echo "$val > 0" | bc -l
with bash arithmetic:
  [ "$val" != "0" ] && [ "$val" != "0.00000000" ]
```
...
This generates a Starknet account and saves it to `.sepolia/account.json`.

### 2. Fund the Account

Fund the generated address with Sepolia ETH:
- **Faucet:** https://faucet.sepolia.starknet.io/

### 3. Deploy Account Contract
...
This deploys all 6 contracts to Sepolia and generates `frontend/.env.testnet`.
...
The frontend is configured for Vercel deployment. Dev tools (`/dev` page and `/api/dev`) are automatically hidden when `NEXT_PUBLIC_NETWORK=testnet`.

### Setup

1. Push the repo to GitHub
2. Import in [Vercel](https://vercel.com) → set root directory to `frontend/`
3. Set environment variables from `frontend/.env.testnet`:
   - `NEXT_PUBLIC_NETWORK=testnet`
   - `NEXT_PUBLIC_STARKNET_RPC_URL=https://starknet-sepolia.g.alchemy.com/starknet/version/rpc/v0_7/demo`
   - `NEXT_PUBLIC_BRIDGE_ADDRESS=<from .env.testnet>`
   - `NEXT_PUBLIC_REGISTRY_ADDRESS=<from .env.testnet>`
   - `NEXT_PUBLIC_POOL_ADDRESS=<from .env.testnet>`
   - `NEXT_PUBLIC_RELAY_ADDRESS=<from .env.testnet>`
   - `NEXT_PUBLIC_WZEC_ADDRESS=<from .env.testnet>`
   - `NEXT_PUBLIC_ORACLE_ADDRESS=<from .env.testnet>`
4. Deploy

The Vercel config is in `frontend/vercel.json`.
